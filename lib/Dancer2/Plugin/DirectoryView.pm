package Dancer2::Plugin::DirectoryView;

=head1 NAME

Dancer2::Plugin::DirectoryView - Browse directory contents in Dancer web apps
...
sub _serve_files {    
    ...
    # Views directory
    my $views_dir = abs_path( $app->config->{views} );
    ...
    if (!File::Spec->file_name_is_absolute($root_dir)) {
        $root_dir = abs_path(catfile(abs_path($app->config->{public_dir} ), $root_dir));
    }
    
    my $real_path = abs_path(catfile($root_dir, $path));
    $real_path =~ s{/$}{};

    if (index($real_path, abs_path($app->config->{public_dir} )) != 0 && !$system_path) {
        # The requested file/directory lies outside of the public directory, but
        # system paths are not allowed
        return $app->send_error("Not allowed", 403);
    }
    ...
    warn "DirectoryView: real_path: $real_path, root_dir: $root_dir, root_url: $root_url, system_path: $system_path";
    ...
    if (-f $real_path) {
        ...
        $app->send_file($real_path, system_path => $system_path);
    }
    elsif (-d $real_path) {
        #
        ...
        for my $name (sort { $a cmp $b } @entries) {
            ...
            my $url = $root_url ? "$root_url/$name" : $name;
        ...
        if ($template) {
            ...
            my $template_simple = Dancer2::Template::Simple->new();
            ...
            if ($options{layout}) {
                ...
                return $template_simple->render($layout_tpl,
                    { listing => $listing_html, path => '/' . $path,
                        path_prefix => $path_prefix, request => $app->request,
                        template => 'default' });
            }
        }
        else {
